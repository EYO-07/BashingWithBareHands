...
function inv {
    ...
    inventory_item 4 "pacman -Qet" "manually installed packages" $width
    inventory_item 5 "pacman -Qm" "AUR repository installed packages" $width
    inventory_item 6 "pacman -Qc <package>" "changelog for package" $width
    inventory_item 7 "fc-cache -fv" "update the font cache" $width
    inventory_item 8 "mkinitcpio -P" "regenerate initramfs manually, kernel or driver updates trigger automatically this command." $width
