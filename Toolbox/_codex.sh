...
function save_variables {
    local __file="$1"; shift
    local dir_part
    dir_part="$(dirname "$__file")"
    # Ensure directory exists
    mkdir -p "$dir_part" || {
        echo "Error: Cannot create directory '$dir_part'."
        return 1
    }
    # Create file if it doesn't exist
    if [[ ! -f "$__file" ]]; then
        touch "$__file" || {
            crit_echo "Error: Cannot create configuration file '$__file'."
            return 1
        }
    fi
    # Always enforce secure owner-only permissions
    chmod 600 "$__file" || {
        echo "Error: Failed to secure permissions on '$__file'."
        return 1
    }
    # Write atomically using a temporary file in the same directory
    local temp_file
    temp_file=$(mktemp "${__file}.XXXXXX") || {
        crit_echo "Error: Cannot create temporary file."
        return 1
    }
    chmod 600 "$temp_file"
    {
        echo "# Generated by save_variables on $(date)"
        for var_name in "$@"; do
            if [ -z "${!var_name+x}" ]; then
                continue
            fi
...
function validate_positive_integer {
    ...
    if [ -z "$value" ]; then
        crit_echo "Error: $name cannot be empty."
        return 1
    fi
    if ! [[ "$value" =~ ^[0-9]+$ ]]; then
        crit_echo "Error: $name must be a positive integer (got: '$value')."
        return 1
    fi
    if [ "$value" -lt 1 ]; then
        crit_echo "Error: $name must be >= 1 (got: '$value')."
        return 1
    fi
    return 0
}
function validate_non_negative_integer { # validate_non_negative_integer <value> [name]
    local value="$1"
    local name="${2:-}"
    if [ -z "$value" ]; then
        crit_echo "Error: $name cannot be empty."
    ...
    if ! [[ "$value" =~ ^[0-9]+$ ]]; then
        crit_echo "Error: $name must be a non-negative integer (got: '$value')."
        return 1
    fi
    return 0
}
function validate_integer_range { # validate_integer_range <value> <min> <max> [name]
    ...
    if ! [[ "$value" =~ ^-?[0-9]+$ ]]; then
        crit_echo "Error: $name must be an integer (got: '$value')."
        return 1
    fi
    if [ "$value" -lt "$min" ] || [ "$value" -gt "$max" ]; then
        crit_echo "Error: $name must be between $min and $max (got: '$value')."
        return 1
    fi
    return 0
}
function validate_url { # validate_url <url> [name]
    ...
    if ! [[ "$url" =~ ^https?://[a-zA-Z0-9.-]+\.[a-zA-Z]{2,}(/.*)?$ ]]; then
        crit_echo "Error: $name does not look like a valid URL (got: '$url')."
        return 1
    fi
    return 0
}
function validate_restricted_path {
    local path="$1"
    local parent="$2"
    # Reject any path containing ".." as a path component (traversal)
    if [[ "$path" =~ (^|/)\.\.(/|$) ]]; then
        return 1
    fi
    # Resolve parent to a canonical absolute path
    local abs_parent
    abs_parent="$(realpath -m "$parent" 2>/dev/null)" || return 1
    # Resolve the target path (relative paths are resolved against parent)
    local abs_path
    if [[ "$path" = /* ]]; then
        abs_path="$(realpath -m "$path" 2>/dev/null)"
    else
        abs_path="$(realpath -m "$parent/$path" 2>/dev/null)"
    fi
    [[ -n "$abs_path" ]] || return 1
    # Ensure the resolved path is inside (or equal to) the parent
    if [[ "$abs_path" != "$abs_parent" && "$abs_path" != "$abs_parent"/* ]]; then
        return 1
    fi
    return 0
}   
